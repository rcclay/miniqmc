...
template<typename DU_TYPE>
void DiracDeterminantRef<DU_TYPE>::invertPsiM(const ValueMatrix_t& logdetT, ValueMatrix_t& invMat)
{
  InverseTimer->start();
...
template<typename DU_TYPE>
void DiracDeterminantRef<DU_TYPE>::resize(int nel, int morb)
{
  int norb = morb;
  if (norb <= 0)
...
template<typename DU_TYPE>
typename DiracDeterminantRef<DU_TYPE>::GradType DiracDeterminantRef<DU_TYPE>::evalGrad(ParticleSet& P, int iat)
{
...
*/
template<typename DU_TYPE>
void DiracDeterminantRef<DU_TYPE>::acceptMove(ParticleSet& P, int iat)
{
  const int WorkingIndex = iat - FirstIndex;
...
template<typename DU_TYPE>
void DiracDeterminantRef<DU_TYPE>::completeUpdates()
{
  UpdateTimer->start();
 ...
 */
template<typename DU_TYPE>
typename DiracDeterminantRef<DU_TYPE>::ValueType DiracDeterminantRef<DU_TYPE>::ratio(ParticleSet& P, int iat)
{
...
template<typename DU_TYPE>
void DiracDeterminantRef<DU_TYPE>::recompute(ParticleSet& P)
{
  SPOVGLTimer->start();
  Phi->evaluate_notranspose(P, FirstIndex, LastIndex, psiM_temp, dpsiM, d2psiM);
  SPOVGLTimer->stop();
